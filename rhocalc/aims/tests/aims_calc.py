...
calcs = {
    ...
    3: {  
        "name": "H2O, periodic, 1 kpt, parallel, n_tasks > n_kpts",
        "atoms": ase.io.read("systems/water_periodic.xyz"),
        "aims_path": aims_path,
        "aims_kwargs": {"k_grid": [1, 1, 1]},
...
def _sum_kso_fields(
    ...
    return np.concatenate([grid[:, :3], np.sum(weighted_ksos, axis=0).reshape(-1, 1)], axis=1)#!/usr/bin/bash
import inspect
import glob
import os
import shutil
